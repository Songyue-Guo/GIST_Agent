...
def validate_structured_output_model_url(model_string: str):
    if "Qwen" in model_string:
        return True
    return False

def validate_chat_model_url(model_string: str):
    if "Qwen" in model_string:
        return True
    return False


def validate_reasoning_model_url(model_string: str):
    if "QwQ" in model_string:
        return True
    return False

def validate_pro_reasoning_model_url(model_string: str):
    return False
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        if 'gpt' in self.model_string:
            self.client = OpenAI(
                api_key=os.getenv("OPENAI_API_KEY"),
                base_url = self.base_url if self.base_url else "https://mdi.hkust-gz.edu.cn/llm_api/v1"
            )
        ...
        # )
        #     self.support_structured_output = validate_structured_output_model_url(self.model_string)
        #     self.is_chat_model = validate_chat_model_url(self.model_string)
        #     self.is_reasoning_model = validate_reasoning_model_url(self.model_string)
        #     self.is_pro_reasoning_model = validate_pro_reasoning_model_url(self.model_string)
